...
Deno.serve(async (req) => {
  try {
    ...
    if (event.type === 'create' && event.entity_name === 'ProgressLog') {
      ...
      if (logs.length === 1) {
        await base44.asServiceRole.entities.Achievement.create({
          client_id, trainer_id, title: "Tracking Started", description: "Logged your first progress update!", badge_icon: "TrendingUp", badge_color: "text-purple-500", category: "milestone", points: 50, earned_date: new Date().toISOString()
        });
      }
